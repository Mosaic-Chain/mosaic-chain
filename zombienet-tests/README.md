...
```bash
$ ./target/release/mosaic-chain build-spec > chain-spec-plain.json
```
```bash
$ ./target/release/mosaic-chain build-spec --chain chain-spec-plain.json --raw > mosaic-chain.json
```
...
### [relaychain]
By default zombienet uses rococo-local testchain, but with changing the default_command and providing a chain spec file to it, it will run the test with mosaic-chain's release build.  
