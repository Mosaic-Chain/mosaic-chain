	...
	impl<T: Config> Pallet<T> {
		...
		fn reward_node(
			...
			let mut staged_details = node_details
				.current()
				.cloned()
				.expect("rewarded nodes must have commited details");
		...
		fn slash_node(
			...
			if !committed_details.own_exposure.currency.is_zero() {
				...
				// FIXME: in the ballances pallet only free balance is considered for slashing.
				// If one's entire balance is staked and thus locked, we dont slash them....
